...
test('notifications normalize legacy Laravel types, submissions, and stable IDs', () => {
  ...
  const alert = normalizeNotification({ id: 'alert-episode', type: 'App\\Notifications\\UsefulAlertNotification', data: { alert_id: 2 } })
  assert.equal(alert.type, 'useful_alert')
  assert.equal(alert.data.type, 'useful_alert')
  assert.equal(alert.data.alert_id, 2)
