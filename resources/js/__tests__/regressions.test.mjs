...
test('mutation refresh invalidates the active snapshot immediately and drops its queued follow-up', () => {
  const { coordinator, clock, requests, states } = autoRefreshFixture()
  coordinator.start()
  coordinator.request(0)
  clock.tick(0)
...
test('mutation refresh replaces scheduled event and retry delays with a new bounded attempt', () => {
  const { coordinator, clock, requests } = autoRefreshFixture()
  coordinator.start()
  coordinator.request()
  ...
  clock.tick(4_000)
  requests[4].complete('error')
  clock.tick(8_000)
  requests[5].complete('error')
  clock.tick(300_000)
  assert.equal(requests.length, 6)
  assert.equal(clock.count(), 0)
})

test('mutation refresh respects navigation and offline suspension and starts once immediately on availability', () => {
...
test('a pending mutation refresh remains inert before start and is cleared on stop', () => {
  const { coordinator, clock, requests } = autoRefreshFixture()
  coordinator.requestFresh()
  clock.tick(1_000)
  assert.equal(requests.length, 0)
