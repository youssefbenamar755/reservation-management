...
<template>
    ...
    <AppLayout
        :breadcrumbs="[{ title: 'Action history', href: '/action-history' }]"
    >
        <div class="flex flex-col gap-6 p-4 sm:p-6 lg:p-8">
                            ...
                            >
                                ...
                                <option
                                    v-if="
                                        ...
                                        !people.some(
                                            (person) =>
                                                person.id === inputs.actor_id,
                                        )
                                    "
                                    :value="inputs.actor_id"
                                >
                                    Selected person (no activity in this view)
                                </option>
                                <option
            ...
            >
                ...
                >
                    <div>
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">
                            {{ history.total.toLocaleString('en-US') }} matching
                ...
                <div v-if="!history.data.length" class="px-6 py-16 text-center">
                    <Inbox class="mx-auto mb-4 size-9 text-muted-foreground" />
                    <h3 class="font-semibold">No actions match this view</h3>
                    <p
                        class="mx-auto mt-2 max-w-md text-sm text-muted-foreground"
                    >
                        Try another filter. New order-status requests, document
                        email sends, webhook retries, and alert actions will
                        appear here.
                    </p>
                </div>
                <ol v-else class="divide-y" :class="{ 'opacity-60': loading }">
                    <li
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            >
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span
                                        ...
                                        >{{ resultLabel(entry.outcome) }}</span
                                    >
                                </div>
                                <time
                                    :datetime="entry.occurred_at ?? undefined"
                                    class="text-xs text-muted-foreground tabular-nums"
                                    >{{ displayDate(entry.occurred_at) }}</time
                                >
                            </div>
                            ...
                            >
                                ...
                                <div class="flex gap-1.5">
                                    ...
                                    <dd class="font-medium">
                                        {{
                                            statusLabel(entry.changes.requested)
                                        }}
                                    </dd>
                                </div>
                                <div
                                ...
                                >
                                    ...
                                    <dd class="font-medium">
                                        {{
                                            statusLabel(entry.changes.confirmed)
                                        }}
                                    </dd>
                                </div>
                            </dl>
                ...
                >
                    ...
                    <div class="flex items-center gap-2">
                        <Button
                            ...
                            @click="
                                visit(props.filters, history.current_page - 1)
                            "
            ...
            <p class="max-w-4xl text-xs leading-relaxed text-muted-foreground">
                Order-status requests and individual email attempts are recorded
                from this feature’s launch. Available earlier email records and
                webhook retries are included. Email and alert activity is
                visible only to the person who performed it; order-status and
                webhook activity follows website access. History refreshes when
                you apply filters or choose Refresh history.
            </p>
        </div>
    </AppLayout>
</template>
