...
test('history requires login and reads metadata without provider calls or payload exposure', function () {
    $this->getJson('/action-history')->assertUnauthorized();
    ...
    $response = $this->actingAs($this->actor)->getJson('/action-history')->assertOk()
    ...
    $this->get('/action-history')->assertInertia(fn (Assert $page) => $page->component('ActionHistory/Index')->has('history.data', 2));
...
test('current website access scopes history and actor options while email and alert actions stay personal', function () {
    ...
    $this->actingAs($this->actor)->getJson('/action-history')->assertOk()->assertJsonPath('history.total', 6)->assertJsonCount(2, 'people')->assertJsonCount(1, 'websites');
    $this->getJson('/action-history?website_id='.$foreignSite->id)->assertForbidden();
    $this->getJson('/action-history?actor_id='.$outsider->id)->assertOk()->assertJsonPath('history.total', 0);
    $this->actingAs($admin)->getJson('/action-history')->assertOk()->assertJsonPath('history.total', 7);
    $this->actingAs($admin)->getJson('/action-history?category=email&actor_id='.$this->actor->id)->assertOk()->assertJsonPath('history.total', 0);
    $this->site->update(['user_id' => $outsider->id]);
    $this->actingAs($this->actor)->getJson('/action-history')->assertOk()->assertJsonPath('history.total', 0)->assertJsonCount(0, 'people');
});
...
test('order links and history do not follow an order transferred to another website', function () {
    ...
    $this->actingAs($this->actor)->getJson('/action-history')->assertOk()->assertJsonPath('history.total', 1)->assertJsonPath('history.data.0.action_url', null);
    $this->getJson('/action-history?order_id='.$this->order->id)->assertNotFound();
    $this->actingAs(User::factory()->create(['is_admin' => true]))->getJson('/action-history?order_id='.$this->order->id)->assertOk()->assertJsonPath('history.total', 0);
});
...
test('per order history avoids ambiguous external webhook ids and applies filters to every page and summary', function () {
    ...
    historyRetry($this->actor, $this->site);
    $this->actingAs($this->actor)->getJson('/action-history?order_id='.$this->order->id)->assertOk()->assertJsonPath('history.total', 2)->assertJsonPath('orderContext.number', 4038);
    $this->getJson('/action-history?category=email&outcome=failed&search=%234038&website_id='.$this->site->id)->assertOk()->assertJsonPath('history.summary', ['total' => 1, 'succeeded' => 0, 'attention' => 1, 'pending' => 0]);
    $this->getJson('/action-history?order_id=999999')->assertNotFound();
});
...
test('date range is inclusive and pagination stays deterministic across mixed sources', function () {
    ...
    historyRetry($this->actor, $this->site);
    $url = '/action-history?start_date=2026-09-13&end_date=2026-09-13';
...
test('abandoned requests become unconfirmed without rewriting history or treating queued retries as failed', function () {
    ...
    $this->actingAs($this->actor)->getJson('/action-history')->assertOk()->assertJsonPath('history.summary', ['total' => 3, 'succeeded' => 0, 'attention' => 1, 'pending' => 2]);
    $this->getJson('/action-history?outcome=uncertain')->assertOk()->assertJsonPath('history.total', 1)->assertJsonPath('history.data.0.finished_at', null);
...
test('deleted orders and users leave readable site history with no broken order link', function () {
    ...
    $admin->delete();
    $this->order->delete();
    $this->actingAs($this->actor)->getJson('/action-history')->assertOk()->assertJsonPath('history.data.0.actor.name', 'Former user')->assertJsonPath('history.data.0.action_url', null);
});

test('history rejects malformed filters', function (string $query) {
    $this->actingAs($this->actor)->getJson('/action-history?'.$query)->assertUnprocessable();
